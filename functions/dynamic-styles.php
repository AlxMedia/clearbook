...
/* ------------------------------------ */
if ( ! function_exists( 'clearbook_hex2rgb' ) ) {

	function clearbook_hex2rgb( $hex, $array=false ) {
		$hex = str_replace("#", "", $hex);
...
/*  Google fonts
/* ------------------------------------ */
if ( ! function_exists( 'clearbook_enqueue_google_fonts' ) ) {

	function clearbook_enqueue_google_fonts () {
		if ( get_theme_mod('dynamic-styles', 'on') == 'on' ) {
			...
			if ( get_theme_mod( 'font' ) == 'inter' ) { wp_enqueue_style( 'inter', '//fonts.googleapis.com/css?family=Inter:400,300,600,800' ); }
...
add_action( 'wp_enqueue_scripts', 'clearbook_enqueue_google_fonts' ); 	


/*  Dynamic css output
/* ------------------------------------ */
if ( ! function_exists( 'clearbook_dynamic_css' ) ) {

	function clearbook_dynamic_css() {
		if ( get_theme_mod('dynamic-styles', 'on') == 'on' ) {
		
			// rgb values
			$color_1 = get_theme_mod('color-1');
			$color_1_rgb = clearbook_hex2rgb($color_1);
			...
			if ( get_theme_mod( 'font' ) == 'inter' ) { $styles .= 'body { font-family: "Inter", Arial, sans-serif; }'."\n"; }
...
#wrapper-inner { background: #fff; }
			...
			// content max-width
			if ( get_theme_mod('content-width','800') != '800' ) {
				$styles .= '
.entry-header,
.entry-footer,
.entry > :not(.alignfull),
...
@media only screen and (max-width: 719px) {
	...
	.toggle-search:focus #svg-close { fill: #fff!important; }
			...
			// social sidebar
			if ( get_theme_mod('color-social-sidebar','#f4f4f4') != '#f4f4f4' ) {
				$styles .= '
.s2 { background: '.esc_attr( get_theme_mod('color-social-sidebar') ).'!important; }
				'."\n";
			}
			// background color
...
body { background: '.esc_attr( get_theme_mod('color-background') ).'; }
			...
			// featured background
			if ( get_theme_mod('color-featured','#e54927') != '#e54927' ) {
				$styles .= '
.book-featured { background: '.esc_attr( get_theme_mod('color-featured') ).'; }
				'."\n";
			}
			...
			if ( get_theme_mod('dark','off') == 'on' ) { 
				wp_add_inline_style( 'clearbook-dark', $styles );
			} else {
				wp_add_inline_style( 'clearbook-style', $styles );
			}
		}
	}
	
}
add_action( 'wp_enqueue_scripts', 'clearbook_dynamic_css' );
